...
class pythonProgramOptions:
    ...
    number_of_systems = 20000  #number of systems per batch
    ...
    if (compas_logs_output_override is None):
        output = os.getcwd()
        output_container = 'Cygnus13results'                 # names the directory to be created and in which log files are created.  Default in COMPAS is "COMPAS_Output"
    else:
    ...
    grid_filename = 'Cygnus13Grid.txt'                                        # grid file name (e.g. 'mygrid.txt')
    ...
    logfile_definitions = 'Cygnus13OutputDefs.txt'                                  # logfile record definitions file name (e.g. 'logdefs.txt')
